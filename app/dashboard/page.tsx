...
export default function DashboardSection() {
  ...
  const fetchMonthlyEntries = async () => {
    ...
    const results = await Promise.all(
      entryTypes.map(async (type) => {
        ...
        console.log(`Monthly count for ${type}:`, count);
        return {
  ...
  const fetchTodayStats = async () => {
    ...
    const getCount = async (type: string) => {
      ...
      const { count, error } = await query;
      if (error) console.error("Error fetching today", type, error);
      console.log(`Today count for ${type}:`, count);
      return count || 0;
    };
      ...
      <div className="relative w-full h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            >
              {data.map((_, i) => (
                <Cell key={i} fill={`url(#color${i})`} stroke="#fff" strokeWidth={2} />
              ))}
            </Pie>
            <Tooltip
